...
Java.perform(function () {
    ...
    const ProxyClass = Java.use('java.net.Proxy');

    // Bypass System.getProperty("http.proxyHost") and "http.proxyPort"
    System.getProperty.overload('java.lang.String').implementation = function (name) {
        if (name.toLowerCase().includes('proxy')) {
            console.log('[Bypass] System.getProperty("' + name + '") → null');
            return null;
        }
        return this.getProperty(name);
    };

    // Bypass android.net.Proxy.getHost()
    Proxy.getHost.overload('android.content.Context').implementation = function (ctx) {
        console.log('[Bypass] Proxy.getHost() → null');
        return null;
    };

    Proxy.getPort.overload('android.content.Context').implementation = function (ctx) {
        console.log('[Bypass] Proxy.getPort() → -1');
        return -1;
    };

    // Bypass new Proxy(Type.HTTP, new InetSocketAddress("proxy", 8080))
    ProxyClass.getType.implementation = function () {
        console.log('[Bypass] java.net.Proxy.getType() → DIRECT');
        return Java.use('java.net.Proxy$Type').DIRECT.value;
    };

    InetSocketAddress.getHostName.implementation = function () {
        console.log('[Bypass] InetSocketAddress.getHostName() → 127.0.0.1');
        return "127.0.0.1";
    };

    InetSocketAddress.getAddress.implementation = function () {
        console.log('[Bypass] InetSocketAddress.getAddress() → null');
        return null;
    };
});
...
*/
Java.perform(function () {
    // Hook the class and method
    var TargetClass = Java.use("com.lcode.shield.MzMtph.qUlPPf.TsDIdY.ClxOYGzO"); // Replace with the actual class name
    TargetClass.detectRootManagementApps.implementation = function () {
        console.log("Bypassing root detection...");

        // Create an empty JSON object to return
        var JSONObject = Java.use("org.json.JSONObject");
        var result = JSONObject.$new();
        result.put("reason", ""); // Set the reason to an empty string

        console.log("Returning empty JSON object to bypass detection.");
        return result; // Return the empty JSON object
    };
});
...
// Frida script to analyze biometric authentication
...
Java.perform(function () {
    ...
    BiometricPrompt.authenticate.overload('android.hardware.biometrics.BiometricPrompt$CryptoObject', 'android.os.CancellationSignal', 'java.util.concurrent.Executor', 'android.hardware.biometrics.BiometricPrompt$AuthenticationCallback').implementation = function (cryptoObject, cancellationSignal, executor, callback) {
        ...
        console.log("[*] AuthenticationCallback: " + callback);

        // Call the original method to proceed with the authentication
        this.authenticate(cryptoObject, cancellationSignal, executor, callback);
    };
